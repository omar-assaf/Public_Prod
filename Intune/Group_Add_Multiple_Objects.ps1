...
{
$DeviceIntuneIDlist = Get-MgDeviceManagementManagedDevice -Filter "serialNumber eq '$SerialNumber'" | select Id 
    if ($null -eq $DeviceIntuneIDlist) 
    {
        Write-Host "Serial Number= $SerialNumber is not enrolled in Intune" -ForegroundColor Yellow
    }
    Else
    {
        ForEach ($DeviceIntuneID in $DeviceIntuneIDlist.Id)
        {
                $EntraDeviceIDlist = Get-MgDevice -CountVariable CountVar -Search deviceId:$DeviceIntuneID -Property "Id"  -ConsistencyLevel eventual
                    ForEach ($DeviceEntraID in $EntraDeviceIDlist.Id)
                    {
                        try
                            {
                             New-MgGroupMember -GroupId $GroupID -DirectoryObjectId $DeviceEntraID
